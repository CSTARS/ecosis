  ...
  request
    ...
    .end(function(err, resp){
      ...
      try {
        ...
        // also, make sure we are using whatever url is specified in config files for resource links
        if( item.ecosis.resources ) {
          item.ecosis.resources.forEach(replaceRootUrl);
        }
...
function replaceRootUrl(resource) {
  if( !resource.url ) return;

  var parts = resource.url.split('/dataset');
  if( parts.length < 2 ) return;
  parts[0] = ckanRoot;

  resource.url = parts.join('/dataset');
}
