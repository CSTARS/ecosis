...
found at http://polymer.github.io/PATENTS.txt
*/
var Ut={distance:function(t,e,i,n){var r=t-i,o=e-n;return Math.sqrt(r*r+o*o)},now:window.performance&&window.performance.now?window.performance.now.bind(window.performance):Date.now};function Zt(t){this.element=t,this.width=this.boundingRect.width,this.height=this.boundingRect.height,this.size=Math.max(this.width,this.height)}function qt(t){this.element=t,this.color=window.getComputedStyle(t).color,this.wave=document.createElement("div"),this.waveContainer=document.createElement("div"),this.wave.style.backgroundColor=this.color,this.wave.classList.add("wave"),this.waveContainer.classList.add("wave-container"),Object(Ct.a)(this.waveContainer).appendChild(this.wave),this.resetInteractionState()}Zt.prototype={get boundingRect(){return this.element.getBoundingClientRect()},furthestCornerDistanceFrom:function(t,e){var i=Ut.distance(t,e,0,0),n=Ut.distance(t,e,this.width,0),r=Ut.distance(t,e,0,this.height),o=Ut.distance(t,e,this.width,this.height);return Math.max(i,n,r,o)}},qt.MAX_RADIUS=300,qt.prototype={get recenters(){return this.element.recenters},get center(){return this.element.center},get mouseDownElapsed(){var t;return this.mouseDownStart?(t=Ut.now()-this.mouseDownStart,this.mouseUpStart&&(t-=this.mouseUpElapsed),t):0},get mouseUpElapsed(){return this.mouseUpStart?Ut.now()-this.mouseUpStart:0},get mouseDownElapsedSeconds(){return this.mouseDownElapsed/1e3},get mouseUpElapsedSeconds(){return this.mouseUpElapsed/1e3},get mouseInteractionSeconds(){return this.mouseDownElapsedSeconds+this.mouseUpElapsedSeconds},get initialOpacity(){return this.element.initialOpacity},get opacityDecayVelocity(){return this.element.opacityDecayVelocity},get radius(){var t=this.containerMetrics.width*this.containerMetrics.width,e=this.containerMetrics.height*this.containerMetrics.height,i=1.1*Math.min(Math.sqrt(t+e),qt.MAX_RADIUS)+5,n=1.1-i/qt.MAX_RADIUS*.2,r=this.mouseInteractionSeconds/n,o=i*(1-Math.pow(80,-r));return Math.abs(o)},get opacity(){return this.mouseUpStart?Math.max(0,this.initialOpacity-this.mouseUpElapsedSeconds*this.opacityDecayVelocity):this.initialOpacity},get outerOpacity(){var t=.3*this.mouseUpElapsedSeconds,e=this.opacity;return Math.max(0,Math.min(t,e))},get isOpacityFullyDecayed(){return this.opacity<.01&&this.radius>=Math.min(this.maxRadius,qt.MAX_RADIUS)},get isRestingAtMaxRadius(){return this.opacity>=this.initialOpacity&&this.radius>=Math.min(this.maxRadius,qt.MAX_RADIUS)},get isAnimationComplete(){return this.mouseUpStart?this.isOpacityFullyDecayed:this.isRestingAtMaxRadius},get translationFraction(){return Math.min(1,this.radius/this.containerMetrics.size*2/Math.sqrt(2))},get xNow(){return this.xEnd?this.xStart+this.translationFraction*(this.xEnd-this.xStart):this.xStart},get yNow(){return this.yEnd?this.yStart+this.translationFraction*(this.yEnd-this.yStart):this.yStart},get isMouseDown(){return this.mouseDownStart&&!this.mouseUpStart},resetInteractionState:function(){this.maxRadius=0,this.mouseDownStart=0,this.mouseUpStart=0,this.xStart=0,this.yStart=0,this.xEnd=0,this.yEnd=0,this.slideDistance=0,this.containerMetrics=new Zt(this.element)},draw:function(){var t,e,i;this.wave.style.opacity=this.opacity,t=this.radius/(this.containerMetrics.size/2),e=this.xNow-this.containerMetrics.width/2,i=this.yNow-this.containerMetrics.height/2,this.waveContainer.style.webkitTransform="translate("+e+"px, "+i+"px)",this.waveContainer.style.transform="translate3d("+e+"px, "+i+"px, 0)",this.wave.style.webkitTransform="scale("+t+","+t+")",this.wave.style.transform="scale3d("+t+","+t+",1)"},downAction:function(t){var e=this.containerMetrics.width/2,i=this.containerMetrics.height/2;this.resetInteractionState(),this.mouseDownStart=Ut.now(),this.center?(this.xStart=e,this.yStart=i,this.slideDistance=Ut.distance(this.xStart,this.yStart,this.xEnd,this.yEnd)):(this.xStart=t?t.detail.x-this.containerMetrics.boundingRect.left:this.containerMetrics.width/2,this.yStart=t?t.detail.y-this.containerMetrics.boundingRect.top:this.containerMetrics.height/2),this.recenters&&(this.xEnd=e,this.yEnd=i,this.slideDistance=Ut.distance(this.xStart,this.yStart,this.xEnd,this.yEnd)),this.maxRadius=this.containerMetrics.furthestCornerDistanceFrom(this.xStart,this.yStart),this.waveContainer.style.top=(this.containerMetrics.height-this.containerMetrics.size)/2+"px",this.waveContainer.style.left=(this.containerMetrics.width-this.containerMetrics.size)/2+"px",this.waveContainer.style.width=this.containerMetrics.size+"px",this.waveContainer.style.height=this.containerMetrics.size+"px"},upAction:function(t){this.isMouseDown&&(this.mouseUpStart=Ut.now())},remove:function(){Object(Ct.a)(this.waveContainer.parentNode).removeChild(this.waveContainer)}},Object(wt.a)({_template:pt.a`
    <style>
      :host {
        display: block;
        position: absolute;
...
<div id="root">
  ...
  <div ?hidden="${this.textMode}">
    ...
    <a id="lastPage" ?selected="${this.lastPage}" @click="${this._selectPage}">${this.lastPageIndex}</a>
  </div>
...
`}.bind(this)}reset(){this.min=this.absMin,this.max=this.absMax}_onMinChange(t){let e=parseFloat(t.currentTarget.value);e>this.max?this.min=this.max:this.min=e,this._fireChange()}_onMaxChange(t){let e=parseFloat(t.currentTarget.value);e<this.min?this.max=this.min:this.max=e,this._fireChange()}_fireChange(){let t=new CustomEvent("range-value-change",{detail:{min:this.min,max:this.max}});this.dispatchEvent(t)}});customElements.define("app-spectra-viewer",class extends(Mixin(st).with(LitCorkUtils)){static get properties(){return{active:{type:Boolean},packageId:{type:String},currentSearchCount:{type:Number},currentIndex:{type:Number},mc1:{type:Array},mc2:{type:Array},mobileFiltersOpen:{type:Boolean},packageTitle:{type:String},freezeAxis:{type:Boolean},minWavelength:{type:Number},maxWavelength:{type:Number},absMinWavelength:{type:Number},absMaxWavelength:{type:Number},minReflectance:{type:Number},maxReflectance:{type:Number},absMinReflectance:{type:Number},absMaxReflectance:{type:Number},yAxisLabel:{type:Boolean},speciesFilters:{type:Array},filterCommonName:{type:Array},filterSpecies:{type:Array},filterGenus:{type:Array},selectedSpeciesFilter:{type:Array}}}constructor(){super(),this.render=function(){return N`
...
Object(wt.a)({is:"paper-icon-button",_template:pt.a`
    ...
    <iron-icon id="icon" src="[[src]]" icon="[[icon]]"
               alt$="[[alt]]"></iron-icon>
  `,hostAttributes:{role:"button",tabindex:"0"},behaviors:[we],registered:function(){this._template.setAttribute("strip-whitespace","")},properties:{src:{type:String},icon:{type:String},alt:{type:String,observer:"_altChanged"}},_altChanged:function(t,e){var i=this.getAttribute("aria-label");i&&e!=i||this.setAttribute("aria-label",t)}});customElements.define("app-header",class extends st{static get properties(){return{}}constructor(){super(),this.render=function(){return N`
...
`}.bind(this)}_onMenuIconClick(){this.dispatchEvent(new CustomEvent("open-menu"))}});customElements.define("ecosis-search-header",class extends(Mixin(st).with(LitCorkUtils)){static get properties(){return{suggestions:{type:Array},filters:{type:Array},text:{type:String}}}constructor(){super(),this.render=function(){return N`
