  ...
  Polymer({
    ...
    filterByWavelength : function() {
        ...
        if( this.showingStats ) {
          ...
          this.dt.addColumn({id: 'tooltip', type: 'string', role: 'tooltip'});
    ...
    toggleStats : function() {
      if( this.$.selectSpectraInput.hasAttribute('disabled') ) {
        // redraw current spectra
        this.showingStats = false;
        this.$.selectSpectraInput.removeAttribute('disabled');
        this.$.selectSpectraInput.classList.remove('disabled');
        this.$.selector.style.color = '#333';
    ...
    createStatsTableArray : function() {
      ...
      for( k in this.stats.value ) {
          ...
          if( this.ptRegex1.exec(k) || this.ptRegex2.exec(k) ) {
            ...
            f = '';
            for( k2 in v ) f += '<b>'+k2+'</b>: '+v[k2].toFixed(6)+'\n';

            this.dataArray.push([parseFloat(k), v.avg, v.min, v.max, f]);
          }
      }
